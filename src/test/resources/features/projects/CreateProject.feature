Feature: Create Project
  ...
  Scenario: Successfully create a project with minimal fields
    ...
    When I create a new project with name "MyNewProject", description "A sample project", visibility "PUBLIC" and status "ToDo"
  ...
  Scenario: Created project is attributed to the authenticated user
    Given I login as "collaborator" with password "password123"
    When I create a new project with name "Attributed Project", description "Ownership check", visibility "PRIVATE" and status "In_Progress"
    Then The response code is 201
  ...
  Scenario: Cannot create a project with a blank name
    Given I login as "collaborator" with password "password123"
    When I create a new project with name "", description "Some description", visibility "PUBLIC" and status "ToDo"
    Then The response code is 400
    And The error message is "must not be blank"

  Scenario: Cannot create a project without authentication
    Given I'm not logged in
    When I create a new project with name "Sneaky Project", description "Should be rejected", visibility "PUBLIC" and status "ToDo"
